...
class WeatherMainBloc extends Bloc<WeathersEvents, WeatherMainState> {
  ...
  WeatherMainBloc(this.repository) : super(WeatherInitial()) {
    on<LoadApiEvent>(
      (event, emit) async {
        emit(WeatherMainLoadingState());
        final activity = await repository.getMainWeather();
    ...
    on<LoadWeatherCard>(
      (event, emit) async {
        ...
        final daily = await repository.getWeatherCardsDaily();
        final hourly = await repository.getWeatherCardsHourly();
        final activity = WeatherCard(
          ...
          precipitationProbabilityMax: daily.daily?.precipitationProbabilityMax,
          ...
          winddirection10MDominant: daily.daily?.winddirection10mDominant,
          lat: 13.41,
          lon: 52.52,
          city: 'Moscow',
          district: 'district',
        ...
        emit(WeatherCardLoadedState(activity));
          ...
          emit(WeatherCardLoadingFailure(e));
        }
